...
export default {
  ...
  methods: {
    onSubmit(evt) {
      ...
      //window.location.replace('http://192.168.178.31:3003/api/login');
      evt.preventDefault();
      axios
        .post(`http://192.168.178.31:3003/api/register`, this.form)
        .then(response => {
  ...
  console.log(evt)
    evt.preventDefault()
    axios.post(`http://192.168.178.31:3003/api/register`, this.form)
    .then(response => {
      // JSON responses are automatically parsed.
      this.form = response.data
    })
